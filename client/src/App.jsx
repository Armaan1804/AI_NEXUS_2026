...
function TypingText({ phrases, typingSpeed = 120, deletingSpeed = 60, pauseTime = 2000 }) {
  ...
  }, [text, isDeleting, phraseIndex, speed, phrases, typingSpeed, deletingSpeed, pauseTime]);
...
function App() {
  ...
  const [deductDraft, setDeductDraft] = useState({ teamId: '', points: '' });
  ...
  const subtitlePhrases = useMemo(() => {
    const rawSubtitle = eventData?.subtitle ?? 'Build | Innovate | Transform';
    return rawSubtitle.split('|').map((phrase) => phrase.trim());
  }, [eventData]);
  ...
  const filteredLeaderboard = [
    { rank: 1, teamName: "Farsight AI", college: "The NorthCap University", teamId: "h-1" },
    { rank: 2, teamName: "Vandalizers", college: "Baddi University of Emerging Sciences and Technology", teamId: "h-2" },
    { rank: 3, teamName: "Rasta", college: "MNNIT Allahabad", teamId: "h-3" },
    { rank: 4, teamName: "404 Trio Not Found", college: "COER University", teamId: "h-4" },
    { rank: 5, teamName: "Ace", college: "Symbiosis Institute of Technology", teamId: "h-5" },
    { rank: 6, teamName: "CloudMind AI", college: "JSPM University", teamId: "h-6" },
    { rank: 7, teamName: "Digital Dominators", college: "Chandigarh University", teamId: "h-7" },
    { rank: 8, teamName: "Elite Engineers", college: "Chandigarh University", teamId: "h-8" },
    { rank: 9, teamName: "Team Cypher", college: "Dr. Akhilesh Das Gupta Institute of Technology and Management", teamId: "h-9" },
    { rank: 10, teamName: "Team Elites", college: "Medhavi Skills University", teamId: "h-10" },
    { rank: 11, teamName: "Team Vayu", college: "Chitkara University", teamId: "h-11" }
  ];
  ...
  async function handleGameDelete(gameId) {
    if (!window.confirm('Are you sure you want to delete this game? All submissions will be lost.')) {
      return;
    }
    ...
    try {
      ...
      setTeams(response.teams);
      setRounds(response.rounds);
      setGameAdminMessage('Game deleted successfully.');
    } catch (deleteError) {
      if (isUnauthorizedAdminError(deleteError)) {
        resetAdminSession();
        setGameAdminError('Admin session expired. Please login again and retry.');
        return;
      }

      setGameAdminError(deleteError.message);
    }
  }

  async function handleDeductPoints(event) {
    event.preventDefault();
    setDeductMessage('');
    setDeductError('');

    if (!adminToken) {
      setDeductError('Please login as admin first.');
      return;
    }

    if (!deductDraft.teamId || !deductDraft.points) {
      setDeductError('Please select a team and enter points.');
      return;
    }

    try {
      const response = await fetchJson(`/api/admin/teams/${deductDraft.teamId}/deduct`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${adminToken}`
        },
        body: JSON.stringify({ points: Number(deductDraft.points) })
      });

      setLeaderboard(response.leaderboard);
      setTeams(response.teams);
      setRounds(response.rounds);
      setDeductDraft({ teamId: deductDraft.teamId, points: '' });
      setDeductMessage('Points deducted successfully.');
    } catch (deductError) {
      if (isUnauthorizedAdminError(deductError)) {
        resetAdminSession();
        setDeductError('Admin session expired. Please login again and retry.');
        return;
      }

      setDeductError(deductError.message);
    }
  }
      ...
      <div className="page-shell admin-page-shell">
        ...
        <main className="content-grid">
          <section className="panel" aria-labelledby="admin-heading" id="admin-editor">
            ...
            {adminToken ? (
              <section className="panel" aria-labelledby="deduct-points-heading" id="deduct-points">
                <div className="section-heading">
                  <p className="eyebrow">Point management</p>
                  <h2 id="deduct-points-heading">Deduct points from teams</h2>
                </div>
                <form className="admin-form" onSubmit={handleDeductPoints}>
                  <div className="admin-block">
                    <div className="admin-row points-row">
                      <label>
                        Select Team
                        <select
                          className="team-select"
                          value={deductDraft.teamId}
                          onChange={(event) => setDeductDraft({ ...deductDraft, teamId: event.target.value })}
                          required
                        >
                          {teams.map((team) => (
                      ...
                      <label>
                        ...
                        <input
                          ...
                          min="1"
                          step="1"
                          value={deductDraft.points}
                          onChange={(event) => setDeductDraft({ ...deductDraft, points: event.target.value })}
                          placeholder="Points"
                          required
                        />
                      </label>
                    ...
                    {deductMessage ? <p className="success-text">{deductMessage}</p> : null}
                    {deductError ? <p className="error-text">{deductError}</p> : null}
                  </div>
                </form>
              </section>
            ) : null}
    ...
    <div className="page-shell">
      ...
      <header className="hero">
        <div className="hero-copy">
          ...
          <h1>
            <TypingText phrases={subtitlePhrases} />
          </h1>
