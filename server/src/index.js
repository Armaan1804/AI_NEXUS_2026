...
import { awardPoints, broadcast, getState, initializeState, subscribeClient, updateScore, deductPoints } from './state.js';
import {
  createGame,
  getGames,
  initializeGames,
...
app.delete('/api/admin/games/:gameId', authenticateAdmin, (request, response) => {
  try {
    const { gameId } = request.params;
    deleteGame(gameId);
    const state = commitAppState();
...
app.post('/api/admin/teams/:teamId/deduct', authenticateAdmin, (request, response) => {
  try {
